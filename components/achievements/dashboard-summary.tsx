 ...
 */
export function AchievementDashboardSummary({ view }: { view: AchievementsView }) {
    ...
    <div className="rounded-3xl border border-border bg-surface-1 p-5 shadow-sm">
      <div className="flex items-center justify-between">
        <p className="flex items-center gap-2 text-xs font-medium uppercase tracking-wide text-neutral-400">
          <Trophy className="size-4 text-accent" /> Trofeeën
        </p>
      ...
      <div className="mt-4 flex items-center gap-4">
        {latest ? (
          <Link href="/member/trophies" className="flex min-w-0 flex-1 items-center gap-3">
            ...
            <span className="min-w-0">
              ...
              <span className="block text-xs text-neutral-500">
                {latest.earnedAt ? DATE_FMT.format(latest.earnedAt) : ""} · {view.earnedCount} totaal
              </span>
            </span>
          </Link>
        ) : (
          <div className="flex min-w-0 flex-1 items-center gap-3">
            ...
            <span className="min-w-0">
              <span className="block font-display font-bold text-neutral-900">Nog geen trofeeën</span>
              <span className="block text-xs text-neutral-500">Start een training om te verdienen</span>
            </span>
          </div>
        )}
      </div>
